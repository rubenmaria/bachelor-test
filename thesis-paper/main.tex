...
\subsection{Methodik} 
Die Parsen der Kommentare wurden mit Tree-sitter realisiert, wie im Abschnitt zur Datenpipeline erwähnt. Dabei gab es eine große Designentscheidung zu treffen.  Welche Kommentare in einer Funktion berücksichtigt werden sollen. 
Im Rahmen dieser Arbeit werden ausschließlich Kommentare verwendet, die direkt  über der Funktion in stehen. Da die einzeiligen Kommentare in der Funktion meistens keinen großen semantischen Wert haben, sondern auf Gefahren oder Designentscheidungen  hinweisen. (Beleg maybe: A survey on Reasearh of code comment). Hierbei muss man aufpassen, dass der Prozess des Parsen nicht zu speziell an die vorliegenden Daten angepasst wird, sonst verliert er seine Allgemeingültigkeit.




Deswegen habe ich mich nicht auf weitere Optimierungen, die ein wenig mehr Kommentare erbringen könnten, eingelassen, sondern es bei den oben beschriebenen belassen.
...
\section{Code2Vec}
\subsection{Motivation} 
Das besondere an \textit{Code2Vec} ist, dass es den Quellcode in eine abstrakten
Syntaxbaum kodiert. Damit nutzt \textit{Code2Vec} alle Quellcodeinformationen die
...
sondern der Semantische Vektroraum wird durch Training des \textit{Code2Vec} Modells
...
einige anpassungen nötig, um \textit{Code2Vec} auf C Quellcode trainieren zu können.
Da das \textit{Code2Vec} Modell auf überwachtes Lernen basiert, muss der Datensatz 
...
Dieses \textit{Code2Vec} Format kann durch eine zusätzlich Option in der Konfiguration
...
den Speicheraufwand stark, aber es wird von \textit{Code2Vec} nicht als valide Eingabe
...
Recall: $64.7$}. Diese Werte sind etwas über den Werten von den \textit{Code2Vec} 
