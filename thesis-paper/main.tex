...
Im folgende wird der Algorithmus skizziert und danach wird auf die potentiellen
gefahren von t-SNE eingegangen. Die hochdimensionalen Datenpunkte werden mit
$\mathbf{H}$ und die niedrigdimensionale Datenpukte mit $\mathbf{N}$ bezeichnet.
Der erste schritt des Algorithmuses ist es 
...
mal wiederholt, wobei $T$ ein Parameter ist der verändert werden kann.
\begin{enumerate}
  \item Berechne Ähnlichkeitsscore von $\mathbf{N}$, diesmal wird die aber die
  ...
  \item Verschiebe die Datenpunkte von $\mathbf{N}$ um ein kleinen Wert in
    die Richtung, die den Unterschied von den Ähnlichkeitsscore von $\mathbf{H}$
