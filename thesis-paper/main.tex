...
Die qualitative Evaluierung wird in dieser Arbeit mittells
\textit{t-SNE} durchgeführt. Dabei wird \textit{t-SNE} verwendet
um die hochdimensionalen Vektoren $x \in \mathbb{R}^{384}$ in 
einen zweidimensionalen Vektorraum zu projizieren. Bei der 
Verwendung und Interpretation von \textit{t-SNE} gibt es 
einiges zu beachten. \\
Zum einen sollten bei der Verwendung mehrere Werte für 
den Perplexity-Parameter $P \in \{2,3,\dots ,50\}$ ausprobiert werden.
Zum anderen kann aus den zweidimensionalen Datenmengen nur 
eine aussage über die Existenz von Clustern abgeleitet werden und
nicht über die position und größe einer Gruppierung. Sogar 
die Existenz eines Clusters in der niedrigdimensionalen
Datenmenge ist zwar ein starkes indiz das ein Cluster
in den hochdimensionalen Daten exisitiert, aber kein Beweis. \\
Im folgenden werden die Werte $P \in \{20, 30, 40\}$, als 
Perplexity-Parameter verwendet, um verschieden Werte auszuprobieren
die jeweils nicht am Rand der empfohlenen Menge $\{2,3, \dots, 50\}$
liegen.\\ 
Um aussagekräftige Graphen zu erhalten wurden $1182$ Funktionen aus
der GNU-Standard-C-Bibliothek in $10$ Kategorien unterteilt.\\
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
Die Grafik liefert bei jeden der Perplexity-Parameter ein 
Indiz dafür, dass die Mathematik und IO Funktionen jeweils größtenteils
in den hochdimensionalen Daten ein Cluster bilden. Sonst ist keine 
weitere Gruppierung zu erkennen. \\
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
Hier gibt es, wie im letzten Abschnitt beschrieben, deutlich weniger
Punkte als bei den restlichen Strategien Embeddings zu erzeugen. In 
den drei Graphen ist eine große Gruppierung in der Kategorie Dateimanagement
und kleine Gruppierungen in den Kategorien Mathematik, String und Signale
zu erkennen. \\
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
In den Graphen ist zu erkennen, dass die Kategorien Nebenläufigkeit,
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
Hier ist festzustellen, dass die Kategorien Mathematik, Nebenläufigkeit,
Speicher und Netzwerk Gruppierungen bilden. Die restlichen Kategorien
bilden zwei oder mehr Untergruppierungen.\\
...
Folgenden die vier verschieden Strategien Embeddings zu erzeugen mit 
$P = 30$ abgebildet, um sie qualitativ zu vergleichen.
\begin{center}
  \begin{tikzpicture}
    \begin{groupplot}[
          group style={
  ...
  \end{tikzpicture}
\end{center}
Die vier durch \textit{t-SNE} generierten Datenmengen, liefern 
ein Indiz, dass die Anzahl der Gruppierungen zunehmen,
in der Reihenfolge:
\textit{Cod2Vec}, Kommentare, Namen und Code-Llama-Erklärungen.
Das heißt auch die qualitative Evaluierung scheint den Trend zu 
folgen das die qualtiät der Embeddings gleichermaßen angeordnet
ist.
