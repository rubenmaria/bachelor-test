...
Das motivierende Beispiel im \textit{Code2Vec} Paper ist die Vorhersage 
...
\begin{figure}[H]
  ...
  \end{center}
  \caption{
    \textit{Code2Vec} Architektur entnommen aus dem Paper
    \cite{alon2018code2veclearningdistributedrepresentations}
  }
\end{figure}
...
ein mächtiges Visualisierungstool sein, um eine Intuition für die
...
\end{figure}
In der Abbildung 3.1 ist die Datenpipeline dargestellt, dabei 
sind Vierecke mit abgerundeten Ecken Tools, die Daten in eine 
andere Darstellung umwandeln, und Vierecke mit spitzen Ecken
repräsentieren Darstellungen von Daten.\\

Die erste Darstellung sind die Rohdaten, die dem unverarbeiteten 
C-Quellcode aus der GNU Standard-C-Bibliothek entsprechen. 
...
In dieser Arbeit wurde {\bf Tree-sitter} als Parser verwendet, welcher
...
Danach gibt es eine Abzweigung, entweder werden die 
Quellinformationen Code Llama nochmal erklärt und dann in den 
SentenceTransformer gegeben oder die Quellinformation wird direkt
in den SentenceTransformer gegeben. Schließlich erhält man wieder 
eine Datenmenge, die aus Tupel besteht. Das Tupel besteht aus dem 
...
Schritt für alle Daten der SentenceTransformer ist, deshalb bildet 
...
untersucht, wie sich der SentenceTransformer bei selber Eingaben 
verhält. Im Optimalfall sollte der SentenceTransfomer bei 
selbem Input dasselbe Ergebnis liefern.\\

Um dies zu überprüfen, wurden $n = ?$ Code-llama 
Quellcode-Erklärungen zufällig ausgewählt und anschließend in
den SentenceTransformer $m = ?$ Mal eingegeben. Dabei beträgt
der höchste Abstand von zwei Vektoren,
die aus der gleichen Erklärung resultiert sind $d = ?$. Der 
Abstand ist ausreichend klein, um ihn in der Evaluation zu 
vernachlässigen. Der SentenceTransformer ist daher für die 
Anwendung in dieser Arbeit ausreichend stabil.
...
\subsection{Methodik} 
Das Parsen der Kommentare wurden mit Tree-sitter realisiert,
...
Paper wird das Modell auch hier darauf trainiert, Funktionsnamen 
...
kodieren, die als Inputformat für maschinelles Lernen dienen.\\

Die implementierung des \textit{Cod2Vec} Modells braucht ein bestimmtes
...
Konfiguration des ASTminers generiert werden. Obwohl diese Option 
mit "code2vec" betitelt ist, ist der Output des ASTminers nicht direkt 
...
Code2Vec nicht als valide Eingabe akzeptiert. Demnach mussten noch die 
...
\verb|CamelCase| und \verb|Snakecase|, normalisiert der \textit{ASTminer} 
...
Um mit Code2Vec diesen Vektor generieren zu können, muss der abstrakte 
...
Cod2Vec mit anderen Ansätzen wie z.b. Funktionskommentaren ist es notwendig,
die normalisierten Namen wieder zu den ursprünglichen Namen zurückzuführen.
Aufgrund dessen mussten im Quellcode von astminer Änderungen vorgenommen 
...
\subsection{Training}
Das Ziel beim Training war es, dieselbe Qualität wie im Paper zu erhalten. 
Also dieselben Ergebnisse für Java auch für C-Quellcode zu replizieren.
Für die Erstellung eines Datensatzes, der aus dem Modell resultiert, können 
bestimmte Problemstellungen beim allgemeinen Trainieren von Modellen 
vernachlässigt werden. Eine Problemstellung beim Trainieren von Modellen 
...
Die Problemstellung muss nicht berücksichtigt werden, da das Ergebnis kein 
...
Informationen des Namens widerspiegeln. Nachdem das Trainingsszenario 
genau dasselbe wie im Paper ist, wurden alle Trainingsparameter gleich 
gelassen. Die Ergebnisse nach der 84 Epoche sind: 
{\bf Precision: $65.6$, F1: $65.1$,  Recall: $64.7$}. Diese Werte sind 
...
und der Test-Datensatz von Code2Vec verschieden ist von dem 
...
Mitleidenschaft gezogen. Damit haben wir ähnliche Ergebnisse wie aus dem
Paper und können diese mit den anderen vorgestellten Ansätzen vergleichen.
