...
die Beziehung gelten, die in der Abbildung 2.3 dargestellt sind.
...
\pagebreak
\hfill\\
Das {\bf Masked Multi Head-Attention}
...
aber auch um Kommentare handeln.\\
\pagebreak
\hfill\\
Der zweitgrößte Teil im Datensatz,
...
\pagebreak
\hfill\\
In Abbildung 3.3 ist der Prozess abgebildet 
...
\]
Das heißt, laut der vorgestellter Formel ist der Embedding-Prozess
  ...
  \begin{center}
    \begin{tikzpicture}
      \begin{groupplot}[
          ...
          \nextgroupplot[title={Funktionskommentare}]
          \addplot[scatter, only marks, scatter src=explicit symbolic]
            table[x=x, y=y, meta=label] {abb/prev-data-tsne-comment-tsne-30.dat};
          \nextgroupplot[title={Funktionsnamen}]
          \addplot[scatter, only marks, scatter src=explicit symbolic]
...
\begin{example}
  ...
  \end{align*}
\end{example}
Da diese Abkürzungen nicht eindeutig zugeordnet werden können,
...
da die Namen fast identisch sind. Im 
Code-Llama-Erklärungen-Raum sind dagegen die 
nächsten Nachbarn von \texttt{lchmod} alle aus der 
...
Die Methodik der 
Funktionskommentare, um Embddings zu erzeugen,
ist, genau wie bei den Funktionsnamen, sehr abhängig vom
Quellcode. Auch die Entscheidung, welche Kommentare man 
einbezieht, kann Auswirkungen auf die Embeddings haben.
In dieser Arbeit wurde nur der Kommentar direkt über der 
Funktion verwendet. In künftiger Forschung könnte untersucht 
werden, ob man auch die Kommentare innerhalb der Funktion 
verwenden kann, um qualitativ hochwertigere Embeddings zu 
produzieren.\\
Bei den Funktionskommentaren ist ebenfalls das Problem, dass der 
Inhalt der Funktion sich nicht immer im Kommentar
widerspiegelt.
\begin{example}
...
beschreibt diese knapp und präzise. Dagegen verweist der 
...
gewählte Modell auch das rechenaufwendigste. In weiterführenden 
Untersuchungen könnten kleinere Modelle mit weniger Rechenaufwand 
verwendet werden, um auf die Frage einzugehen, ob der Rechenaufwand 
eines Code-Llama-Modells mit 70 Milliarden Parametern nötig ist. \\
...
einer künftigen Forschung eine Untersuchung über die 
Stabilität von Large Language-Models durchgeführt werden.\\ 
Schließlich könnte bei weiteren Untersuchungen ein anderer
Prompt ausprobiert werden. Hier kann bspw. der
Prompt von Wang et al. verwendet werden 
\cite{clap}. Dadurch könnten sich die Erklärungen 
verbessern und damit in Folge die Qualität der 
Embeddings.\\\\
\textit{Code2Vec}. 
Embeddings mit dieser Methode zu erzeugen,
...
um die Embeddings zu generieren, groß sein. \\
...
enstehen dadurch dieselben Probleme wie bei der Diskussion zu 
den Funktionsnamen.\\
...
Perplexity-Parameter in \textit{t-SNE}. In künftiger Forschung 
...
Das wäre auch wünschenswert, da die Formel für 
alle $k = 2,3,\dots, N$ auszurechnen einen hohen Rechenaufwand 
darstellt. Das liegt primär an dem K-Nearest-Neighbor-Algorithmus 
bzw. an einer Funktion, die Nachbarn berechnet und aufsteigend 
nach dem Abstand zum eingegebenen Punkt sortiert. Die restlichen 
...
\[
  \text{score}_k(l,i,v) = \begin{cases*} 
    ...
    G_k := \sum_{i=1}^{k} \frac{1}{log_2(i+1)}.
\]
All diese Verallgemeinerungen regen das
Ausprobieren verschiedener Instanzen der verallgemeinerten 
Objekte an. In künftiger Forschung könnte herausgefunden  
werden, welche Vorteile und Nachteile die verschiedene 
Instanzen haben.\\\\
\textit{Code2Vec und BERT als Inspiration für ein neues Modell.} 
Meiner Meinung nach sind die größten Probleme von Code2Vec zum
einen die Abhängigkeit von den Funktionsnamen und zum anderen 
das überwachte Lernen, das eine Aufbereitung der Daten erfordert.
Anstatt das Modell darauf zu trainieren,
Funktionsnamen vorherzusagen, könnte man mit unüberwachtem Lernen,
wie in dem BERT-Modell, Trainingsaufgaben erteilen, welche das 
Verständnis von Quellcode stärken. Da der Input von 
\textit{Code2Vec} ein abstrakter Syntaxbaum ist, kann hier eine 
Aufgabe sein, einen fehlenden Knoten im Baum vorherzusagen. Eine 
weitere Trainingsaufgabe kann sein, ob zwei Zweige in 
einem abstrakten Syntaxbaum sequenziell hintereinander kommen oder 
nicht. Dazu könnte man noch entweder das Prinzip von Aufmerksamkeit
...
Inhalt einer Funktion codieren. Zum einen wurden aus Funktionsnamen, 
...
Anhand einer Expertenbefragung konnten die besten Embeddings
...
gefolgt von Funktionsnamen, danach Funktionskommentare 
...
können direkt eingesetzt werden, um viele verschiedene
Problemstellungen für die C-Programmiersprache zu lösen.\\
Die Motivation dieser Arbeit war es aber, Vektoren für 
ein neuronales Netzwerk zu generieren, um dieses neuronale Netzwerk,
...
Reverse Engineering \cite{reverse-engeneering},
oder das Klassifizieren von 
Maleware in Maschinensprache\cite{maleware-detection}. 
Dieses neuronales Netzwerk kann nun mit den Vektoren,
