...
def generate_vectors(input_dir: str, output_name: str) -> dict[str,NDArray]:
...
def retrieve_vectors_from_files(output_name: str) -> dict[str,NDArray]:
    ...
    return {name : vector for name in names for vector in vectors}
...
def parse_vector_file(path: str) -> list[NDArray]:
    ...
    with open(path) as file:
        for line in file:
            vectors.append(
                numpy.array(list(map(lambda x: float(x), line.split())))
            )
    return vectors
